...
let make backend { Dao.Client_vif.domid; device_id; } ~ip ~client_ip =
  ...
  let (_my_mac, _our_mac) as mac =
    Client_ethernet.mac ethernet, Netbackend.frontend_mac backend in
  let (_my_ip, _our_ip) as ip = ip, client_ip in
  Lwt.return { mac; ip; ethernet; domid }

let pp ppf { ip= (_, ip); domid; _ } = Fmt.pf ppf "dom%d:%a" domid Ipaddr.V4.pp ip
